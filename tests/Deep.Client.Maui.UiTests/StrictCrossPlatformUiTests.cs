...
public sealed class StrictCrossPlatformUiTests
{
    ...
    private static void AttachToExistingProvisionedClients(CrossPlatformOptions options)
    {
        ...
        {
            var terminal = windows.FindAutomationId("PhysicalE2E.RuntimeReadyMarker")?.Name ?? "missing";
            throw new InvalidOperationException($"Windows runtime sync did not become ready; state={terminal}.");
        }
...
internal sealed class AndroidUiautomatorClient
{
    ...
    private StrictCrossPlatformContracts.AndroidNode WaitByText(string resourceId, string text, TimeSpan timeout) { var until = DateTime.UtcNow + timeout; Exception? last = null; while (DateTime.UtcNow < until) { try { return StrictCrossPlatformContracts.FindExactlyOneVisibleResourceIdWithText(Dump(), resourceId, text); } catch (Exception ex) { last = ex; } Thread.Sleep(250); } throw new InvalidOperationException($"Required Android resource-id/text pair did not reach its expected state: {resourceId}.", last); }
