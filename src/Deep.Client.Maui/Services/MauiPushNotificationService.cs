...
#if WINDOWS
                return ResolveWindowsTokenAsync(cancellationToken);
#elif ANDROID
...
#if WINDOWS
        private static async Task<string?> ResolveWindowsTokenAsync(CancellationToken cancellationToken)
        {
                var channel = await Windows.Networking.PushNotifications.PushNotificationChannelManager.CreatePushNotificationChannelForApplicationAsync();
                cancellationToken.ThrowIfCancellationRequested();
                return string.IsNullOrWhiteSpace(channel.Uri) ? null : channel.Uri;
        }
#endif
}
